...
## quantiles
quants <- c(0.7, 0.75, 0.8, 0.85, 0.9,0.95)
...
bio_h_summary<-  expand.grid(biol.endpoints=biol.endpoints,flow.endpoints=flow.endpoints, 
                              quants = quants, stringsAsFactors = F)
...
gam.lm <-lapply(1:nrow(bio_h_summary), function(i)
{
  ...
  qgam(MetricValue~s(deltah_final, bs="ad"), 
       data = mydat, 
        qu = qmet)
...
for(i in 1:length(gam.lm)) {
  ...
  incr <- mean(mydat$deltah_final)/400
  ...
  DFX <- cbind(newdata, as.data.frame(predictedVals)) %>%
    ...
    mutate(Variable = tmet,  Metric = bmet, Quant = qmet) %>%
    distinct()
...
for(b in 1:length(bio)) {
  ...
  for(m in 1:length(mets)) {
    ...
    T1 <- (ggplot(subset(DF1, Variable == mets[m]) %>% arrange(predictedVals), aes(y=predictedVals, x=deltah_final, group = Quant, col = Quant)) +
             ...
             geom_hline(yintercept = 0.64,  linetype="dashed", linewidth=0.5, color = "grey50") +
