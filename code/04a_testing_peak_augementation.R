...
P1 <- ggplot() +
  ...
  facet_wrap(~Flow.Metric.Name, scales = "free") 
  # scale_x_continuous(name=paste(mets[m])) +
  # scale_y_continuous(name = paste0("Index Score  (", bio[b], ")"))

P1

file.name1 <- paste0(out.dir, "04a_raw_data_plot_CSCI.jpg")
...
file.name1 <- paste0(out.dir, "04a_raw_data_plot_ASCI.jpg")
ggsave(P2, filename=file.name1, dpi=300, height=5, width=7.5)

## remove augmented peaks - can cite RB9 tech report 
...
## save models
save(gam.lm, file = "output_data/04_csci_asci_quantile_gam_noPeakAugs.RData")
gam.lm
## load
# load(file = "output_data/04_csci_asci_quantile_gam_noPeakAugs.RData")
...
save(bio_h_summary, file="output_data/04_csci_asci_quantile_gam_rsqds_noPeakAugs.RData")

## load
# load(file="output_data/04_csci_asci_quantile_gam_rsqds_noPeakAugs.RData")
bio_h_summary
